class u_socket {
    constructor(auth_data = false, f = false, websock = false) {
        ...
        this.on('server.connected', async () => {
    ...
    async consume_sctp(room, id, socket, user) {
        ...
        consumer.on('transportclose', () => user.consumers.delete(consumer.id))
        consumer.on('close', () => user.consumers.delete(consumer.id))
        consumer.on('open', () => { })
    ...
    async init_transports(room) {
        ...
        room.producerTransport.on('producedata', async ({ sctpStreamParameters, label, protocol, appData }, callback, errback) => {
            let producer_id
        ...
        room.consumerTransport.on('connectionstatechange', async (state) => {
            switch (state) {
                case 'connecting':
                    break
                case 'connected':
    ...
    strip_html(data) {
        return data.toString().replace(/[\u200B-\u200D\uFEFF]/g, '').replace(/(<([^>]+)>)/gi, "").replace('<', '&lt;').replace('>', '&gt;')
    }
    ...
    connect(protocol = '-', config) {
        ...
        if (!config) config = config = {
            ssl: location.href.indexOf('https') !== -1,
            port: 443,
    ...
    close(code) {
        ...
        if (code !== 4666) setTimeout(() => location.reload(), 3000)
    }
    ...
    async on(cmd, data) {
        try {
            if (typeof data === 'function') {
                ...
                if (this.on_functions['*']) this.on_functions['*'].map(f => p.push(new Promise(async r => r(await f([cmd, data])))))

                if (this.on_functions[cmd]) this.on_functions[cmd].map(f => p.push(new Promise(async r => r(await f(data)))))
